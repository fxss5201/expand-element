<template>
  ...
  <div class="scroll-box" ref="scroll">
    <div style="height: 100px;background-color: aqua;">Scroll</div>
  </div>
  <div>
...
import { ref } from 'vue'
// import { getIsScrollY } from 'expand-element'
import { getIsScrollY } from '../../../../lib/main'
...
const noScrollFn = () => {
  if (noScroll.value) {
    const res = getIsScrollY(noScroll.value)
    noScrollCode.value = JSON.stringify(res, null, 2)
  }
}
const scrollFn = () => {
  if (scroll.value) {
    const res = getIsScrollY(scroll.value)
